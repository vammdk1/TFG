...
ruta_lectura = elementos[3]
ruta_guardado = elementos[4]
diccionario = {}
...
try:
    ...
    with open(ruta_lectura+"/"+elementos[1], 'r') as archivo:
        ...
        if len(elementos)> 1 :
            ...
            existing_files = os.listdir(ruta_guardado)
            if existing_files:
                # Elimina el archivo existente
                existing_file_path = os.path.join(ruta_guardado, existing_files[0])
                os.remove(existing_file_path)

            print("Guardado en :"+ruta_guardado+'/IMG.png')
            rutaSave = os.path.join(ruta_guardado, "IMG.png")
            plt.savefig(rutaSave)
            #plt.show()
            plt.clf()
